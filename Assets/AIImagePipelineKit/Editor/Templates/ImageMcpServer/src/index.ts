...
type ReferenceRole = z.infer<typeof ReferenceRoleSchema>;
...
type GenerateImageResult = {
  ...
  metadataPath: string;
  stage: "preview" | "final";
  usedReferenceImagePaths: string[];
};

type ImageGenerationJob = {
  jobId: string;
  status: "pending" | "running" | "preview_succeeded" | "succeeded" | "failed";
  request: GenerateImageInput;
  previewResult?: GenerateImageResult;
...
function getDefaultSize(provider: Provider): string {
  if (provider === "volcengine")
...
function getDefaultPreviewSize(provider: Provider): string {
  if (provider === "volcengine")
...
function getProxyUrl(provider: Provider): string | undefined {
  if (provider === "volcengine")
...
function createProxyFetch(proxyUrl?: string): any {
  if (!proxyUrl)
    return undefined;
...
/** 创建图像客户端。 */
function createImageClient(provider: Provider): OpenAI {
...
function ensureInsideOutputRoot(targetPath: string): string {
...
async function createVersionedFilePath(folder: string, baseName: string, extension: string): Promise<string> {
...
async function persistJob(job: ImageGenerationJob): Promise<void> {
  await ensureDir(JOBS_ROOT);
  await writeJson(getJobPath(job.jobId), job);
}

function persistJobSoon(job: ImageGenerationJob): void {
...
async function loadJob(jobId: string): Promise<ImageGenerationJob | undefined> {
  ...
  const job = await readJson<ImageGenerationJob>(filePath);
  imageJobs.set(job.jobId, job);
  return job;
}

async function listPersistedJobs(): Promise<ImageGenerationJob[]> {
  ...
  for (const file of files.filter(fileName => fileName.endsWith(".json"))) {
    try {
      jobs.push(await readJson<ImageGenerationJob>(path.join(JOBS_ROOT, file)));
    } catch {
...
async function downloadToFile(url: string, filePath: string, provider: Provider): Promise<void> {
  const proxyFetch = createProxyFetch(getProxyUrl(provider));
...
async function resolveReferenceImagePath(referencePath: string): Promise<string> {
...
async function resolveReferenceImagePaths(referenceImages?: ReferenceImage[]): Promise<string[]> {
  ...
  for (const reference of referencesWithPath)
    resolved.push(await resolveReferenceImagePath(reference.path as string));

  return resolved;
}

function buildPrompt(input: { style?: string; usage?: string; assetType?: string; prompt: string; referenceImages?: ReferenceImage[] }): string {
...
function createImageRequest(input: { provider: Provider; model: string; prompt: string; size: string; quality: Quality; responseFormat: ResponseFormat }): any {
...
async function requestOpenAIImageWithOptionalReferences(input: { client: OpenAI; model: string; prompt: string; size: string; quality: Quality; referenceImagePaths: string[] }): Promise<any> {
  ...
  return (input.client.images as any).edit({
    model: input.model,
    image: imageInput,
    prompt: input.prompt,
    size: input.size,
...
async function generateAndSaveImage(input: GenerateImageInput, stage: "preview" | "final" = "final"): Promise<GenerateImageResult> {
  ...
  if (input.provider === "openai") {
    ...
    result = await client.images.generate(requestBody as any, {
      extraBody: {
...
function startImageGenerationJob(input: GenerateImageInput): ImageGenerationJob {
...
async function promoteImageGenerationJob(jobId: string): Promise<ImageGenerationJob> {
...
function serializeJob(job: ImageGenerationJob): Record<string, unknown> {
  return {
    jobId: job.jobId,
    status: job.status,
...
server.tool(
  "health_check",
  "Return image MCP configuration and safety limits without generating images.",
  {},
  async () => {
    return {
      content: [
...
server.tool(
  "save_asset_manifest",
  "Validate and save a Unity UI image asset manifest. This does not generate images.",
  {
...
server.tool(
  ...
  "Generate one Unity UI image asset synchronously. Prefer start_image_generation_job for Codex.",
  ImageGenerateToolSchema,
  async ({ provider, model, assetName, prompt, outputFolder, style, usage, assetType, size, quality, responseFormat, watermark, referenceImages, previewFirst, previewSize, previewQuality, finalSize, finalQuality }) => {
...
server.tool(
  ...
  "Start one image generation job asynchronously and return jobId immediately. Supports reference images and preview-first workflow.",
  ImageGenerateToolSchema,
  async ({ provider, model, assetName, prompt, outputFolder, style, usage, assetType, size, quality, responseFormat, watermark, referenceImages, previewFirst, previewSize, previewQuality, finalSize, finalQuality }) => {
...
server.tool(
  ...
  "Get the status/result/error of an async image generation job.",
  {
...
server.tool(
  "promote_image_generation_job",
  "Promote a preview-succeeded job into final high-quality generation.",
  {
    jobId: z.string().min(1)
  },
  async ({ jobId }) => {
...
server.tool(
  ...
  "List recent async image generation jobs in this MCP server process.",
  {},
  async () => {
...
server.tool(
  "generate_assets_from_manifest",
  "Generate images from a saved manifest synchronously. Use only after user approval.",
  {
...
server.tool(
  ...
  "List generated PNG assets under a relative folder inside IMAGE_MCP_OUTPUT_ROOT.",
  {
...
server.tool(
  ...
  "Resolve local reference image paths relative to the Unity project root or IMAGE_MCP_OUTPUT_ROOT. This does not generate images.",
  {
